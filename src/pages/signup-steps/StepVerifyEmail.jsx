      ...
      <div className="flex gap-3 mb-6">
        {code.map((d, i) => (
          <input key={i} ref={(el) => (refs.current[i] = el)} type="text" inputMode="numeric" maxLength={1}
            ...
            className={`w-12 h-14 text-center text-xl font-semibold rounded-xl border bg-gray-90 text-white outline-none transition-colors ${d ? 'border-blue-60' : 'border-[#2C2F36]'} focus:border-blue-60`}
          />
        ))}
      </div>
      <button onClick={timer <= 0 ? () => setTimer(30) : undefined} disabled={timer > 0}
        className={`w-full h-14 rounded-full font-semibold text-[0.9375rem] transition-colors mb-6 ${timer > 0 ? 'bg-gray-90 text-gray-60 cursor-not-allowed' : 'bg-blue-60 hover:bg-[#1a5cff] text-white cursor-pointer'}`}>
