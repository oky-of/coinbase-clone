...
const MarketStatCard = ({ label, value, change, statId }) => {
  ...
  return (
    <Link
      to="/market-stats"
      className="flex-1 min-w-60 h-35 bg-gray-5 rounded-lg p-4 hover:shadow-elevation-1 transition-shadow duration-200 cursor-pointer overflow-hidden relative border border-transparent hover:border-gray-10 block">
      ...
      <div className="absolute left-0 bottom-0 w-full h-15">
        <StatChart
          changePct={change}
...
function _buildSmoothPath(points) {
...
const _getCoinBrandColor = (symbol) => {
  const colors = {
    BTC: "#F7931A",
    ETH: "#627EEA",
    BNB: "#F3BA2F",
        ...
        <Container className="py-8 md:py-12">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <aside className="w-full lg:w-[320px] shrink-0 flex flex-col gap-6 lg:pl-8 lg:border-l lg:border-gray-10">
              {/* Get Started CTA */}
              <div className="bg-blue-60 rounded-2xl p-5 relative overflow-hidden min-h-40">
                <div className="relative z-10 w-[65%] shrink-0">
                  <h3 className="text-[17px] leading-6 font-semibold text-white mb-1">
                    Get started
                  </h3>
                  <p className="text-[15px] leading-5 text-white mb-5">
                ...
                <div className="absolute right-0 top-1/2 -translate-y-1/2 h-full flex items-center justify-end pointer-events-none">
                  <img
                    ...
                    className="w-31 h-31 object-contain translate-x-2"
                  />
                </div>
              </div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {loading
                    ? Array.from({ length: 2 }).map((_, i) => (
                        <div
                          key={i}
                          className="flex-1 min-w-30 bg-gray-5 rounded-xl p-4 h-30 animate-pulse"
                        />
                      ))
                    : topGainers
                        .slice(0, 4)
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {loading
                    ? Array.from({ length: 2 }).map((_, i) => (
                        <div
                          key={i}
                          className="flex-1 min-w-35 bg-gray-5 rounded-xl p-4 h-30 animate-pulse"
                        />
                      ))
                    : newCoins.map((coin) => (
