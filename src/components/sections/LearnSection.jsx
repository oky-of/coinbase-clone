...
const articles = [
	{
		...
		description:
			'Stablecoins like USDC are often used to move value across crypto markets while staying pegged to the US dollar. They are a common starting point for understanding digital payments and onchain finance.',
		image:
...
const ArticleCard = ({ article }) => (
	<div className="relative flex flex-col min-h-80 min-w-65 m-5 overflow-hidden ">
		<a
			href={article.href}
			target="_blank"
			rel="noopener noreferrer"
			aria-label={article.title}
			className="absolute inset-0 z-10 transition-opacity duration-150 hover:opacity-[0.88]"
		/>
		...
		<div className="flex flex-col gap-3 pt-4 flex-1 px-1">
			<h3 className="text-2xl leading-6   line-clamp-3 m-0">
				{article.title}
			</h3>
		...
		<div className="w-full max-w-400 px-6 py-12 md:px-8 md:py-16 md:max-w-307 lg:px-12 lg:py-20 lg:max-w-400">
			<div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8 mb-10 md:mb-12">
				...
				<div className="flex flex-col items-start gap-6 w-full md:max-w-lg">
					...
					<Link
						to="/learn"
						className="inline-flex items-center justify-center h-14 rounded-full bg-gray-100 text-white font-semibold text-xl  px-8 leading-5 no-underline transition-opacity duration-200 hover:opacity-[0.88] whitespace-nowrap"
					>
						Read More
					</Link>
				</div>
